...
struct Qwen2Layers {
  ...
  // [dim + 2*kv_dim, hidden_dim] instead of three. fused_qkv_enabled_ is
  // false when LLAMA_DISABLE_FUSED_QKV=1 or the model is quantized.
  // The *_src_ tensors keep the fused weight/bias buffers alive
  // (set_weight/set_bias store non-owning external views into them).
