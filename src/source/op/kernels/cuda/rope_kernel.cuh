...
namespace kernel {
// CUDA RoPE on raw bfloat16 q / k; the sin/cos caches are stored as bf16
// (filled by sin_cos_cache_calc_cu, widened exactly on load).
...
// One launch per (layer, K/V). Raw bf16 element copy.
...
// Fill the sin/cos caches (bf16 storage, fp32 fill math rounded to bf16):
