...
std::string fill_template(const std::string& content) {
  const std::string fmt = "<|im_start|>user\n%s<|im_end|>\n<|im_start|>assistant\n";
  std::string result = fmt;
  size_t pos = result.find("%s");
  if (pos != std::string::npos) result.replace(pos, 2, content);
  return result;
}
...
BenchmarkResult benchmark_batch(std::shared_ptr<model::Model> model,
  ...
  for (int i = 0; i < num_requests; ++i) {
    const std::string& raw = prompts[i % prompts.size()];
    std::string sentence = fill_template(raw);
  ...
  // Flush any pending CUDA errors before touching host data
  {
    cudaError_t err = cudaDeviceSynchronize();
    if (err != cudaSuccess) {
      LOG(ERROR) << "[BENCH] CUDA error after benchmark loop: " << cudaGetErrorString(err);
    }
  }
  auto end = Clock::now();
...
int main(int argc, char* argv[]) {
  ...
  auto model = std::make_shared<model::Qwen3Model>(
  ...
  LOG(INFO) << "Warming up...";
  {
    ...
    LOG(INFO) << "[BENCH] GPU before warmup: free=" << (free_mem >> 20)
              << "MB total=" << (total_mem >> 20) << "MB";
  }
  {
    scheduler::Scheduler warm_sched(model, max_batch, 128, std::min(max_gen, 8));
    auto tokens = model->encode(fill_template("warm up"));
    if (tokens.empty()) tokens = {1};
  ...
  cudaDeviceSynchronize();
  {
    size_t free_mem, total_mem;
    cudaMemGetInfo(&free_mem, &total_mem);
    LOG(INFO) << "[BENCH] GPU after warmup, before free_idle: free=" << (free_mem >> 20)
              << "MB total=" << (total_mem >> 20) << "MB";
  }
  base::CUDADeviceAllocatorFactory::get_instance()->free_idle();
  {
    size_t free_mem, total_mem;
    cudaMemGetInfo(&free_mem, &total_mem);
    LOG(INFO) << "[BENCH] GPU after free_idle: free=" << (free_mem >> 20)
              << "MB total=" << (total_mem >> 20) << "MB";
  }
